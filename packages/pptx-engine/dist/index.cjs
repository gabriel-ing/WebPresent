...
__export(index_exports, {
  ...
  buildSlideState: () => buildSlideState,
  clampCrop: () => clampCrop,
  escHtml: () => escHtml,
  fillToCss: () => fillToCss,
  getCroppedImageStyles: () => getCroppedImageStyles,
  parsePptx: () => parsePptx,
  renderParagraph: () => renderParagraph,
  renderShape: () => renderShape,
  renderTextRun: () => renderTextRun
});
...
function renderParagraph(para) {
  ...
  const bullet = para.bulletType === "bullet" ? `<span style="margin-right:0.4em">${escHtml(para.bulletChar || "\u2022")}</span>` : "";
...
function buildSlideState(slide, animationStep, mediaResolver = () => "") {
  return {
    ...
    shapesHtml: (slide.shapes || []).map((s) => renderShape(s, animationStep, mediaResolver)).join("\n")
  };
}
...
function parseTheme(xmlStr, parser) {
  if (!xmlStr) return { colours: {}, defaultFont: void 0 };
  ...
  if (gradFill) {
    ...
    if (gsLst) {
      ...
      const stops = list.map((gs) => ({
        ...
        // OOXML positions are in 1/1000ths of %
...
function parseBorder(spPr, themeColours) {
  ...
  if (!widthEmu) return void 0;
  const widthPt = emuToPt(widthEmu);
  const colour = extractColour(solidFill, themeColours);
  if (!colour) return void 0;
  ...
  if (!xfrm) {
    ...
    if (off2 || ext2) {
      return {
        x: emuToPx(off2?.["@_x"]),
        y: emuToPx(off2?.["@_y"]),
        width: emuToPx(ext2?.["@_cx"]),
        height: emuToPx(ext2?.["@_cy"]),
        rotation: 0
      };
    }
...
async function parsePptx(filePath, deckId, getDeckDir) {
  ...
  let themeData = { colours: {}, defaultFont: void 0 };
  try {
  ...
  const slidesOutputDir = import_node_path.default.join(getDeckDir(deckId), "slides");
  await import_promises.default.mkdir(slidesOutputDir, { recursive: true });
  const mediaMap = /* @__PURE__ */ new Map();
  let mediaSeq = Date.now();
  async function extractMedia(pptxInternalTarget, slideSubDir) {
    const resolved = import_node_path.default.posix.normalize(import_node_path.default.posix.join(`ppt/${slideSubDir}`, pptxInternalTarget));
    if (mediaMap.has(resolved)) return mediaMap.get(resolved);
    const entry = zip.getEntry(resolved);
    if (!entry) return void 0;
    const ext = import_node_path.default.extname(resolved).toLowerCase() || ".png";
    const outName = `pptx-media-${mediaSeq}${ext}`;
    mediaSeq++;
    const outPath = import_node_path.default.join(slidesOutputDir, outName);
    await import_promises.default.writeFile(outPath, entry.getData());
  ...
  function extractMediaFromShapes(shapes, dir) {
    for (const shape of shapes) {
      if (shape.imageTarget) {
        const resolved = import_node_path.default.posix.normalize(import_node_path.default.posix.join(`ppt/${dir}`, shape.imageTarget));
        if (!mediaMap.has(resolved)) {
          ...
          if (entry) {
            ...
            mediaSeq++;
            const outPath = import_node_path.default.join(slidesOutputDir, outName);
            (0, import_node_fs.writeFileSync)(outPath, entry.getData());
      ...
      if (shape.fill?.imageTarget) {
        const resolved = import_node_path.default.posix.normalize(import_node_path.default.posix.join(`ppt/${dir}`, shape.fill.imageTarget));
        if (!mediaMap.has(resolved)) {
          const entry = zip.getEntry(resolved);
          if (entry) {
            const ext = import_node_path.default.extname(resolved).toLowerCase() || ".png";
  ...
  function extractMediaFromBackground(background, dir) {
    if (background?.imageTarget) {
      const resolved = import_node_path.default.posix.normalize(import_node_path.default.posix.join(`ppt/${dir}`, background.imageTarget));
      if (!mediaMap.has(resolved)) {
        const entry = zip.getEntry(resolved);
        if (entry) {
          const ext = import_node_path.default.extname(resolved).toLowerCase() || ".png";
  ...
  function parseLayoutFile(layoutFullPath, layoutDir, layoutBaseName) {
    ...
    try {
      ...
      try {
        ...
        if (masterRel) {
          masterPath = import_node_path.default.posix.normalize(import_node_path.default.posix.join(`ppt/${layoutDir}`, masterRel.target));
        }
      } catch {
      }
      const bg = cSld?.["p:bg"];
  ...
  function parseMasterFile(masterFullPath) {
    ...
    try {
      ...
      const masterDir = import_node_path.default.posix.dirname(masterFullPath.replace(/^ppt\//, ""));
      const masterBaseName = import_node_path.default.posix.basename(masterFullPath);
      let masterRels = {};
      try {
  ...
  for (let i = 0; i < slidePaths.length; i++) {
    ...
    const slideDir = import_node_path.default.posix.dirname(slidePath);
    const slideBaseName = import_node_path.default.posix.basename(slidePath);
    let slideXml;
    try {
    ...
    if (layoutRel) {
      const layoutPath = import_node_path.default.posix.normalize(import_node_path.default.posix.join(`ppt/${slideDir}`, layoutRel.target));
      const layoutDir = import_node_path.default.posix.dirname(layoutPath.replace(/^ppt\//, ""));
      const layoutBaseName = import_node_path.default.posix.basename(layoutPath);
  ...
  return {
    sourceFileName: import_node_path.default.basename(filePath),
    slides,
    theme: themeData
  };
}
